...
class OrchestratorLogger:
    ...
    def _build_context_suffix(
        ...
        """
        Build a context suffix string for structured log entries.

        Appends available context fields as a pipe-delimited suffix so
        every log line optionally carries debugging identifiers.
        ...
        Returns:
            Context suffix string like " | req=abc user=xyz agent=123"
            or empty string if no context provided
        """
        parts = []
        if request_id:
        ...
        if agent_id:
            parts.append(f"agent={str(agent_id)[:8]}")
        if orchestrator_id:
            parts.append(f"orch={str(orchestrator_id)[:8]}")
    ...
    def error_with_context(
        ...
        """
        Log an error with structured debugging context.

        Attaches request_id, user_id, agent_id, and orchestrator_id to the log
        line so operators can correlate errors to specific requests and entities.
        ...
        """
        ctx = self._build_context_suffix(request_id, user_id, agent_id, orchestrator_id)
        self.logger.error(f"[red]❌ {message}{ctx}[/red]")
    ...
    def warning_with_context(
        ...
        """
        Log a warning with structured debugging context.
        ...
        """
        ctx = self._build_context_suffix(request_id, user_id, agent_id, orchestrator_id)
        self.logger.warning(f"[yellow]⚠️  {message}{ctx}[/yellow]")
    ...
    def info_with_context(
        ...
        """
        Log an info message with structured debugging context.
        ...
        """
        ctx = self._build_context_suffix(request_id, user_id, agent_id, orchestrator_id)
        self.logger.info(f"[cyan]{message}{ctx}[/cyan]")
