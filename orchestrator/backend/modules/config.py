...
# ============================================================================
# RAPIDS WORKSPACE/PROJECT CONTEXT
# ============================================================================
...
_active_archetype: Optional[str] = None
_project_working_dirs: Dict[str, str] = {}  # project_id -> repo_path
...
def set_active_workspace(workspace_id: str) -> None:
    ...
    config_logger.info(f"Active workspace set: {workspace_id}")
...
def set_active_project(project_id: str, repo_path: str,
                       name: str = None, archetype: str = None) -> None:
    """
    Switch the active project context.
    Also updates the working directory for backward compatibility.
    """
    global _active_project_id, _active_project_name, _active_archetype
    _active_project_id = project_id
    _active_project_name = name
    _active_archetype = archetype
    _project_working_dirs[project_id] = repo_path
    # Update legacy working dir for backward compatibility
    set_working_dir(repo_path)
    config_logger.info(f"Active project set: {name or project_id} ({archetype}) at {repo_path}")
...
def get_active_archetype() -> Optional[str]:
    """Get the active project's archetype."""
    return _active_archetype


def get_project_dir(project_id: str) -> Optional[str]:
    """Get working directory for a specific project."""
    return _project_working_dirs.get(project_id)


def register_project_dir(project_id: str, repo_path: str) -> None:
    """Register a project's working directory without switching to it."""
    _project_working_dirs[project_id] = repo_path


def get_all_project_dirs() -> Dict[str, str]:
    """Get all registered project working directories."""
    return dict(_project_working_dirs)
...
def clear_project_context() -> None:
    ...
    config_logger.info("Project context cleared")


# RAPIDS Plugin Configuration
RAPIDS_PLUGINS_DIR = os.getenv(
    "RAPIDS_PLUGINS_DIR",
    str(Path(__file__).parent.parent.parent.parent / ".claude" / "rapids-plugins"),
)

# Workspace directory
WORKSPACE_DIR = os.getenv(
    "WORKSPACE_DIR",
    str(Path(__file__).parent.parent.parent.parent / "workspace"),
)
