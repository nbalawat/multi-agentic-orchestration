...
const getAgentLogCount = (
  ...
  const isOrchestrator = agent.metadata?.is_orchestrator || agent.name === 'O-Agent';
  const events = store.eventStreamEntries.filter(
    (event) => {
      if (isOrchestrator) {
        // Match orchestrator events by agentName or sourceType
        return event.agentName === 'O-Agent' ||
          event.sourceType === 'orchestrator_chat' ||
          event.sourceType === 'thinking_block' ||
          event.sourceType === 'tool_use_block';
      }
